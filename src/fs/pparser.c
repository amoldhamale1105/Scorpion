...
{
    int len = strnlen(filename, SCORPION_MAX_PATH);
...
{
    char* result_path_part = kzalloc(SCORPION_MAX_PATH);
    int i = 0;
    ...
    if (**path == '/')
    {
        //Skip forward slash to avoid issues while parsing the next path part
        *path += 1;
    }
...
struct path_root* pathparser_parse(const char* path, const char* current_dir_path)
{
    ...
    if (strlen(path) > SCORPION_MAX_PATH)
    {
        goto out;
    }
