...
#define SCORPION_FAT16_SIGNATURE 0x29
#define SCORPION_FAT16_FAT_ENTRY_SIZE 0x02
#define SCORPION_FAT16_BAD_SECTOR 0xFF7
#define SCORPION_FAT16_UNUSED 0x00
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != SCORPION_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
    ...
    while (1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != SCORPION_ALL_OK)
        {
            res = -EIO;
            goto out;
        }
        ...
        {
            //We have a blank record which means end of root dir
            break;
        }

        //Is the item unused. Directory entry is available (free)
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    {
        //To fetch the correct number of sectors in case of an offset
        total_sectors++;
    }
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != SCORPION_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != SCORPION_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if(diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != SCORPION_ALL_OK)
    {
        res = -EIO;
        goto out;
    }

    if (fat_private->header.shared.extended_header.signature != SCORPION_FAT16_SIGNATURE)
    {
        res = -EFSNOTUS;
        goto out;
    }

    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != SCORPION_ALL_OK)
    {
        res = -EIO;
        goto out;
    }
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * SCORPION_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        {
            // We are at last entry in the file
            res = -EIO;
            goto out;
        }

        //Check if sector is maked as bad
        if (entry == SCORPION_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[SCORPION_MAX_PATH];
    for (int i = 0; i <directory->total; i++)
    {
        fat16_get_full_relative_filename(&directory->item[i], tmp_filename, sizeof(tmp_filename));
        if (istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0)
        {
            //found file. Let's create a new fat_item
