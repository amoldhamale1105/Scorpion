...
static bool elf_valid_class(struct elf_header* header)
{
    ...
    return header->e_ident[EI_CLASS] == ELFCLASSNONE || header->e_ident[EI_CLASS] == ELFTCLASS32;
}

static bool elf_valid_encoding(struct elf_header* header)
{
    //Set the encoding to little endian (LSB at the lowest address space) only
...
{
    return header->e_type == ET_EXEC && header->e_entry >= SCORPION_PROGRAM_VIRTUAL_ADDRESS;
}
...
{
    return (char*) header + elf_section(header, header->e_shtrndx)->sh_offset;
}
